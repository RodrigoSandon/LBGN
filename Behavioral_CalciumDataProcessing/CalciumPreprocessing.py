...
def main():
    ...
    for count, i in enumerate(gpio_file_paths):
        ...
        """Add gpio_start_time to all abet values in columns dealing with time 
        """

        """ NO LONGER NEEDED
        preprocessed_dff = pd.read_csv(preprocessed_dff_filepaths[count])

        gpio_filtered_dff_df = util.delete_certain_rows_of_df(
            preprocessed_dff, gpio_start_time
        )
        print(gpio_filtered_dff_df.head())
        print(roots[count])
        gpio_filtered_dff_df.to_csv(
            os.path.join(roots[count], "dff_traces_preprocessed_gpio.csv"), index=False
        )
        """
